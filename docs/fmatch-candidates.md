# Current fmatch candidates

The demo candidate catalog is available from
`GET /api/debug/acquirers` on the backend at `:8080`. The backend seeds ten
fictional solvers at startup; they are test data, not real providers.

| Slug | Name | Geography | Limits | Example pair fees |
| --- | --- | --- | --- | --- |
...
Detailed API-layer, quality, and latency information is available at
`GET /api/debug/acquirers/<slug>`, for example
`/api/debug/acquirers/vanda`.
