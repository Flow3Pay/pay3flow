...
Важно: Cow Protocol Services не заменяет `fmatch`. В нашей архитектуре
`fmatch` остаётся рабочим matcher'ом solver'ов через ActivityPub. Cow нужен
как reference для orderbook, auction window, solver competition, выбора
победителя и settlement/status/proof flow.

Итоговая роль компонентов:

```text
backend/orderbook
  -> fmatch получает задачу и возвращает solver candidates
